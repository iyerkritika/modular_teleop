...
//        move_robot.publish
        ...
        joint_torque[0] = torque(0,0)/100;
        joint_torque[1] = torque(1,0)/100;
        joint_torque[2] = torque(2,0)/100;
...
{
    ros::init(argc,argv,"geomagic_interface");
    ros::NodeHandle nh;

    feedback obj(nh);
    ros::Rate freq(100);
   pub1 = nh.advertise<geometry_msgs::Twist>("/irb120/end_vel",10);
    HDErrorInfo error;
